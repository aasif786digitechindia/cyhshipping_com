...
class LiveSiteControlProvider extends ServiceProvider {
	...
	public function boot() {
		...
		$gdl_enabled_option        = get_option( 'gdl_enabled' );
		$gdl_site_published_option = get_option( 'gdl_site_published' );

		add_action(
			is_admin() ? 'admin_enqueue_scripts' : 'wp_enqueue_scripts',
			function( $hook_suffix ) use ( $gdl_enabled_option, $gdl_site_published_option ) {
				...
				wp_localize_script(
					...
					array(
						'page'                   => $hook_suffix,
						'appContainerClass'      => empty( $gdl_enabled_option ) ? null : self::APP_CONTAINER_CLASS,
	...
	public function maybe_publish_site_on_admin_init() {
		...
		// if the gdl_enabled option is missing we can safely assume some destructive action was taken and publish the site.
		if ( empty( $wpnux_imported_option ) && empty( $gdl_enabled_option ) && empty( $gdl_site_published_option ) ) {
	...
	public function gdl_launch_site_notice() {
		if ( empty( get_option( 'gdl_enabled' ) ) ) {
			return;
		}
