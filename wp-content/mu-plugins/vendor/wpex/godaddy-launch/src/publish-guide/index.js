/* global gdlPublishGuideItems, gdvPublishGuideDefaults */
/**
...
import { getLiveSiteControlProperty, hasLiveSiteControlData, isMigratedSite } from '../common/utils/gdlLiveSiteControlDataHelpers';
...
export function PublishGuide() {
	...
	useEffect( () => {
		...
		setTimeout( function() {
			const actions = {
				'add-site-logo': 'site_media',
				'edit-site-title': 'site_info',
			};

			if ( actionParam in actions ) {
				setSelectedItem( actions[ actionParam ] );
			}
		}, 1200 );

		if ( actionParam === 'add-site-logo' ) {
			const observer = new MutationObserver( function() {
				if ( ! document.body.classList.contains( 'modal-open' ) && document.contains( document.getElementById( 'gdl-add-logo' ) ) ) {
					document.getElementById( 'gdl-add-logo' ).click();
					observer.disconnect();
				}
			} );
	...
	const [
		...
		sitePublished = ( ! hasLiveSiteControlData() ),
	] = useEntityProp( 'root', 'site', getLiveSiteControlProperty( 'settings' )?.publishState );
				...
				{ actions: [
					{
						...
						onClick: () => {
							...
							window.dispatchEvent( new Event( getLiveSiteControlProperty( 'eventName' ) ) );
						},
					},
