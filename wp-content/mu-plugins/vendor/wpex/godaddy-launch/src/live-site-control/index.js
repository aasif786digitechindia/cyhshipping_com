...
import { getLiveSiteControlProperty, isMigratedSite } from '../common/utils/gdlLiveSiteControlDataHelpers';
import './index.scss';
...
const LiveSiteControl = () => {
	...
	const [ , setSitePublish ] = useEntityProp( 'root', 'site', getLiveSiteControlProperty( 'settings' )?.publishState );
	const [ , setBlogPublic ] = useEntityProp( 'root', 'site', getLiveSiteControlProperty( 'settings' )?.blogPublic );
	...
	const { isWelcomeGuideActive } = useSelect( ( select ) => {
		if ( getLiveSiteControlProperty( 'page' ) !== 'post-new.php' ) {
			return false;
		}
	...
	const removeGlobalEventListeners = useCallback( () => {
		window.removeEventListener( getLiveSiteControlProperty( 'eventName' ), launchWorkflow );
	}, [] );
	 ...
	 */
	useEffect( () => {
		window.addEventListener( getLiveSiteControlProperty( 'eventName' ), launchWorkflow );
	...
	const workflow = {
		confirm: {
			...
			props: {
				isCaaSGenerated: getLiveSiteControlProperty( 'isCaaSGenerated' ) === 'true',
				isCompleted,
				onCloseModal: () => {
					closeModal();
		...
		success: {
			...
			props: {
				closeModal: () => {
					closeModal();
					setTimeout( () => {
						unmountComponentAtNode( document.getElementById( getLiveSiteControlProperty( 'appContainerClass' ) ) );
		...
		if ( isCompleted ) {
			return (
				<LiveSitePublishGuideLaunchContinueButton
					launchWorkflow={ () => {
						closePublishGuide();
						getLiveSiteControlProperty( 'isCaaSGenerated' ) === 'true' ? launchWorkflow() : milestonePublishNow(); // eslint-disable-line no-unused-expressions
					} }
				/>
			);
		}
...
domReady( () => {
	const rootElement = document.getElementById( getLiveSiteControlProperty( 'appContainerClass' ) );

	if ( ! rootElement ) {
		return;
	}

	if ( getLiveSiteControlProperty( 'shouldUseReact18Syntax' ) === 'true' ) {
